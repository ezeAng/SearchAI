...
class Board:
    ...
    def print_board_goal_walk(self, path_taken):
        threat_locs = self.get_threatened_locations()
        top_bot_line = ""
        x_axis = "    "
        for i in range(self.cols+1):
            top_bot_line += "__"
        ...
        for i in range(self.rows):
            ...
            for j in range(self.cols):
                ...
                if col_item < 0:
                    to_add = 'X'
                    string += to_add
                elif (i,j) in path_taken:
                    to_add = 'g'
                    string += to_add
                elif (i,j) in threat_locs:
                    to_add = 'T'
                    string += to_add
                else:
                    if self.is_goal(i,j):
...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    while len(frontier) > 0:
        ...
        if (current.state.x, current.state.y) in new_board.goals:
        ...
        for action in actions(current.state, grid, rows, cols):
            if action not in no_go_zone and ((action) not in reached):
    ...
    # print(new_board.print_board_goal_walk(paths_r_c(soln_path)))
    # print(soln_path)
    return (soln_path)
